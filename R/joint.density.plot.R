...
joint.density.plot <- function(x, y, Title=NULL, contour=TRUE, color=FALSE)
     ...
     if(length(y) != length(x))
          stop("vectors x and y must be the same length in contour.plot().")
     if(any(!is.finite(x)))
          stop("x has missing or infinite values in contour.plot().")
     if(any(!is.finite(y)))
          stop("y has missing or infinite values in contour.plot().")
